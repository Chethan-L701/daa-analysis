...
template <typename T> auto sel_sort(vector<T> &vec) {
    auto start = std::chrono::high_resolution_clock::now();
    Selection::sort(vec);
    auto end = std::chrono::high_resolution_clock::now();
    auto dur =
...
template <typename T> auto merge_sort(vector<T> &vec) {
    auto start = std::chrono::high_resolution_clock::now();
...
template <typename T> auto quick_sort(vector<T> &vec) {
    auto start = std::chrono::high_resolution_clock::now();
...
template <typename T> int benchmark(int range) {
    ...
    for (int i = 1; i <= range; ++i) {
        ...
        uvec.clear();
        uvec = get_ranvec(i);
        vector<T> ssort(uvec), msort(uvec), isort(uvec), qsort(uvec),
            bsort(uvec);
        file << sel_sort(ssort) << ',' << bubble_sort(bsort) << ','
             << ins_sort(isort) << ',' << merge_sort(msort) << ','
             << quick_sort(qsort) << '\n';
    }
    return 0;
}
