...
template <typename T> bool Sort::sort_check(vector<T> &vec) {
...
template <typename T> void Sort::Selection::sort(vector<T> &vec) {
...
template <typename T> void Sort::Bubble::sort(vector<T> &vec) {
    for (int i = 0; i < vec.size(); ++i) {
...
template <typename T> vector<T> Sort::Merge::sort(vector<T> vec) {
    if (vec.size() <= 1)
        return vec;
    int mid = vec.size() / 2;
...
template <typename T> void Sort::Insertion::sort(vector<T> &vec) {
